...
(function($) {
  ...
  $.paletteColorPicker = function( el, options ) {
    ...
    plugin.init = function() {
      ...
      if (plugin.settings.TFSOverload == true) {
        $button.addClass("TFSOverload");
        $bubble.addClass("TFSOverload");
      }
      ...
      .on(click_handler, '.'+ns+'-bubble span.swatch', function(e){
        ...
        if (plugin.settings.TFSOverload == true) {
          $('[name="' + $button.attr('data-target') + '"]').trigger('change');
        }
        
      })['insert'+plugin.settings.insert]( $el );

      // Upside / downside, default is upside
      if ( plugin.settings.position === 'downside') {
